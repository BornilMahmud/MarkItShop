...
const orderValidation = {
  validatePaymentMethod: (paymentMethod) => {
    ...
    if (!allowedMethods.includes(normalized)) {
      throw new ValidationError(`Payment method must be one of: ${allowedMethods.join(', ')}`, 'paymentMethod');
    }

    return normalized;
  },

  validatePaymentPhone: (paymentPhone) => {
    if (!paymentPhone || typeof paymentPhone !== 'string') {
      throw new ValidationError('Mobile banking number is required', 'paymentPhone');
    }

    const digits = paymentPhone.replace(/[^0-9]/g, '');
    if (digits.length < 11 || digits.length > 15) {
      throw new ValidationError('Mobile banking number must be 11 to 15 digits', 'paymentPhone');
    }
  ...
  validateTransactionId: (transactionId) => {
    ...
    if (normalized.length < 6 || normalized.length > 50) {
      throw new ValidationError('Transaction ID must be 6 to 50 characters', 'transactionId');
    }

    if (!/^[a-zA-Z0-9\-]+$/.test(normalized)) {
      throw new ValidationError('Transaction ID can contain only letters, numbers, and hyphen', 'transactionId');
    }
  ...
  validateStatus: (status) => {
    ...
    return status.toLowerCase() === 'cancelled' ? 'canceled' : status.toLowerCase();
  }
};
...
const validateOrderData = (orderData) => {
  ...
  validatedData.paymentMethod = safeValidate(orderValidation.validatePaymentMethod, orderData.paymentMethod || 'bkash', 'paymentMethod');
  validatedData.paymentPhone = safeValidate(orderValidation.validatePaymentPhone, orderData.paymentPhone, 'paymentPhone');
  validatedData.transactionId = safeValidate(orderValidation.validateTransactionId, orderData.transactionId, 'transactionId');
