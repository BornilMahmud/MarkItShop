...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (sessionStatus === "authenticated") {
      router.replace((session as any)?.user?.role === "admin" ? "/admin" : "/");
    }
  }, [sessionStatus, router, searchParams, (session as any)?.user?.role]);
        ...
        <div className="mt-5 sm:mx-auto sm:w-full sm:max-w-[480px]">
          <div className="bg-white px-6 py-12 shadow sm:rounded-lg sm:px-12">
            ...
            <div className="mt-8 rounded-md bg-slate-50 p-4 text-sm text-slate-600">
              <p>Use your customer account or the configured admin credentials to access the dashboard.</p>
              <p className="mt-2">
                Need access?{" "}
