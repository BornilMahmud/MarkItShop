        ...
        <div>
          <label className="form-control w-full max-w-xs">
            ...
            <select
              ...
              value={order?.paymentMethod || "bkash"}
          ...
          <div>
            <label className="form-control w-full max-w-xs">
              ...
              <input
                type="text"
                className="input input-bordered w-full max-w-xs"
                value={order?.paymentPhone || ""}
                onChange={(e) => setOrder({ ...order, paymentPhone: e.target.value })}
              />
            </label>
          </div>
          <div>
        ...
        <div>
          {orderProducts?.map((product) => (
            <div className="flex items-center gap-x-4" key={product?.id}>
              <Image
                src={toDisplayImageSrc(product?.product?.mainImage)}
              ...
              <div>
                ...
                <p>
                  {formatBDT(product?.product?.price)} * {product?.quantity} items
                </p>
              </div>
            </div>
          ))}
          <div className="flex flex-col gap-y-2 mt-10">
            <p className="text-2xl">Subtotal: {formatBDT((order?.total || 0) - (order?.shippingCharge || SHIPPING_CHARGE_BDT))}</p>
            <p className="text-2xl">Shipping: {formatBDT(order?.shippingCharge || SHIPPING_CHARGE_BDT)}</p>
            <p className="text-3xl font-semibold">
              Total: {formatBDT(order?.total || 0)}
            </p>
          </div>
