...
const SearchPage = async ({ searchParams }: Props) => {
  ...
  const searchValue = Array.isArray(sp?.search) ? sp.search[0] : sp?.search || "";
  let products = [];

  try {
    const data = await apiClient.get(`/api/search?query=${searchValue}`);
      ...
      <div className="max-w-screen-2xl mx-auto">
        {searchValue && (
          <h3 className="text-4xl text-center py-10 max-sm:text-3xl">
            Showing results for {sanitize(searchValue)}
          </h3>
        )}
